...
JumpingLegLength = 0.8;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


%MAX IS 0.4 BUT CHECK THAT IS LESS THAN MAXSTRESS/FORCE
CompressionDistance = 0.30;

JumpingLegCompressedLength = JumpingLegLength-CompressionDistance;
JumpingLegStartAngle = rad2deg(asin(LandingLegHeight/JumpingLegCompressedLength));

% Bar.Length can vary betwee5 0.5 and 0.7 m
Bar.Length0             = 0.5;  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Bar.MaxCompressionRatio = (Bar.Length0-CompressionDistance)/Bar.Length0;
...
%Bar.I           = ((Bar.Radius)^3)*Bar.Width/12; %%%%%%%%% for rectangular
Bar.I           = Bar.Radius^4*pi/4; %CIRCULAR

Bar.Density     = 1600;
Bar.Yield       = 2.05e9;
Bar.Number      = 2; %fixed

Bar.Bundle      = 7; %number of bars
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Bar.WidthOffset  = 18e-3;
Bar.LengthOffset = 10e-3; %not sure
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Bar.Euler       = Bar.Number*Bar.Bundle*pi^2*Bar.E*Bar.I/Bar.Length0^2;
P_Eu            = Bar.Number*Bar.Bundle*pi^2*Bar.E*Bar.I/Bar.Length0^2;
Bar.Resolution  = 30; %20 is fine, 30 gives a more precise measurement
Bar.CompressionSteps    = 30;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Bar.Length(j) = Bar.Length(j) - Bar.WidthOffset*sin(Bar.Theta(1,j)) - Bar.LengthOffset*2;                
end


Bar.Force = abs(mean(diff(Bar.Energy(2:end))./diff(Bar.Length(2:end))))
massima = max(abs(diff(Bar.Energy(2:end))./diff(Bar.Length(2:end))))

maxStress = max(Bar.MaxStress/Bar.Yield)
realStress = maxStress * 0.8

Energy = max(Bar.Energy)

subplot(1,2,1)
plot(Bar.X(:,Bar.CompressionSteps),Bar.Y(:,Bar.CompressionSteps),'ro-')
hold on
xlabel('X (m)')
ylabel('Y (m)')
axis equal
